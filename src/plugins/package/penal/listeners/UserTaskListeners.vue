  ...
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="small" border>
      ...
      <el-table-column label="操作" width="90px">
        <template #default="scope">
          <el-button size="small" type="primary" link @click="openListenerForm(scope.row, scope.$index)">编辑</el-button>
          <el-divider direction="vertical" />
          <el-button size="small" type="primary" link style="color: #ff4d4f" @click="removeListener(scope.row, scope.$index)">移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="listenerFormModelVisible" title="任务监听器" :size="`${width}px`" append-to-body destroy-on-close>
      ...
      <p class="listener-filed__title">
        <span
          ><i> <Menu class="panel-tab-icon__title" /> </i>注入字段：</span
        >
