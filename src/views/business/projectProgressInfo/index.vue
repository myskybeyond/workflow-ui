<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['business:projectProgressInfo:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['business:projectProgressInfo:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['business:projectProgressInfo:remove']"
              >删除</el-button
            >
          </el-col>
      ...
      <el-table v-loading="loading" :data="projectProgressInfoList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="260px">
          <template #default="scope">
            ...
            <el-tooltip content="设计" placement="top">
              <el-button link type="primary" icon="Setting" @click="handleEditExcel(scope.row)"></el-button>
            </el-tooltip>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="projectProgressInfoFormRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="项目" prop="projectId">
        ...
        <el-form-item prop="col">
          <template #label>
            列
            <el-tooltip effect="dark" content="用于统计excel中的进度信息" placement="top">
              <el-icon><WarningFilled /></el-icon>
            </el-tooltip>
          </template>
          <el-input v-model="form.col" placeholder="请输入列信息" />
        </el-form-item>
        <el-form-item label="下单日期" prop="orderDate">
          <el-date-picker clearable v-model="form.orderDate" type="date" value-format="YYYY-MM-DD" placeholder="请选择下单日期"> </el-date-picker>
        </el-form-item>
        <el-form-item label="开始发货日期" prop="deliverStartDate">
          <el-date-picker clearable v-model="form.deliverStartDate" type="date" value-format="YYYY-MM-DD" placeholder="请选择开始发货日期">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="发货结束日期" prop="deliverEndDate">
          <el-date-picker clearable v-model="form.deliverEndDate" type="date" value-format="YYYY-MM-DD" placeholder="请选择发货结束日期">
          </el-date-picker>
        </el-form-item>
...
<script setup name="ProjectProgressInfo" lang="ts">
import { listProjectProgressInfo, getProjectProgressInfo, delProjectProgressInfo, addProjectProgressInfo, updateProjectProgressInfo, synProjectProgressInfo } from '@/api/business/projectProgressInfo';
import { ProjectProgressInfoVO, ProjectProgressInfoQuery, ProjectProgressInfoForm } from '@/api/business/projectProgressInfo/types';
...
const projectProgressInfoFormRef = ref<ElFormInstance>();
...
const initFormData: ProjectProgressInfoForm = {
  id: undefined,
  projectId: undefined,
  orderId: undefined,
  name: undefined,
...
const data = reactive<PageData<ProjectProgressInfoForm, ProjectProgressInfoQuery>>({
  form: {...initFormData},
  queryParams: {
    pageNum: 1,
    pageSize: 10,
...
const getList = async () => {
  ...
  const res = await listProjectProgressInfo(queryParams.value);
  projectProgressInfoList.value = res.rows;
...
const reset = () => {
  ...
  projectProgressInfoFormRef.value?.resetFields();
}
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: ProjectProgressInfoVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加项目进度信息";
  form.value.col = "R";
}

/** 修改按钮操作 */
const handleUpdate = async (row?: ProjectProgressInfoVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getProjectProgressInfo(_id);
  await getOrdersByProjectId(res.data.projectId);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改项目进度信息";
}

/** 提交按钮 */
const submitForm = () => {
  projectProgressInfoFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id) {
        await updateProjectProgressInfo(form.value).finally(() =>  buttonLoading.value = false);
      } else {
        //创建在线文件信息
        const fileForm = {
          id:'',
        ...
        await addProjectProgressInfo(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: ProjectProgressInfoVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除项目进度信息编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delProjectProgressInfo(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('business/projectProgressInfo/export', {
    ...queryParams.value
  }, `projectProgressInfo_${new Date().getTime()}.xlsx`)
}
...
const handleEditExcel = async(row?:any) => {
  ...
  EventBus.$on("excelFileInfo", async function(eventObj: { version: any; name: any; id: any; }) {
    ...
    if(onlineFile.value.onlineFileId == id){
      ...
      await updateProjectProgressInfo( onlineFile.value.row);
      await getList();
    }
  });
  //编辑文档
...
const handleSync = async(row?:any) => {
  //同步流程信息
  let query = {id: row.id};
  const  res = await synProjectProgressInfo(query);
