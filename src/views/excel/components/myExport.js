...
export async function exportSheetExcel(luckysheet, name = 'file') {
  return new Promise((res) => {
    ...
    luckysheet.forEach(function (table) {
      ...
      // console.log(table.name, '开始时间1', timestamp, '时间差', Date.now() - timestamp);
      // timestamp = Date.now();
      setStyleAndValue(table.data, worksheet);
      // console.log(table.name, '开始时间2', timestamp, '时间差', Date.now() - timestamp);
      // timestamp = Date.now();
      setMerge(table.config.merge, worksheet);
      // console.log(table.name, '开始时间3', timestamp, '时间差', Date.now() - timestamp);
      // timestamp = Date.now();
      setBorder(table, worksheet);
      // console.log(table.name, '开始时间4', timestamp, '时间差', Date.now() - timestamp);
      // timestamp = Date.now();
      setImages(table, worksheet, workbook);
      // console.log(table.name, '开始时间5', timestamp, '时间差', Date.now() - timestamp);
      // timestamp = Date.now();
      return true;
    });
    // 4.写入 buffer
...
var setImages = function (table, worksheet, workbook) {
  ...
  for (let key in images) {
    ...
    if (visibledatarow) console.log('图片名', myBase64Image, col_st, row_st);
...
var setStyleAndValue = function (cellArr, worksheet) {
  ...
  cellArr.forEach(function (row, rowid) {
    ...
    //todo 2024年4月26日 行高和行宽设置会有点慢，可以加开关控制速度
    //设置单元格行高,默认乘以1.2倍
