...
export async function exportSheetExcel(luckysheet, name = 'file') {
  ...
  luckysheet.every(function (table) {
    ...
    setStyleAndValue(table.data, table.config, worksheet);
...
var setStyleAndValue = function (cellArr, config, worksheet) {
  if (!Array.isArray(cellArr)) return;

  cellArr.forEach(function (row, rowid) {
    const dbrow = worksheet.getRow(rowid + 1);
    //设置单元格行高,默认乘以1.2倍
    if (config && config.rowlen && config.rowlen[rowid]) {
      dbrow.height = config.rowlen[rowid] * 1.2;
    }
    row.every(function (cell, columnid) {
      if (!cell) return true;
      // if (rowid == 0) {//todo 后面可以优化一下性能
      const dobCol = worksheet.getColumn(columnid + 1);
      //设置单元格列宽除以8
      if (config && config.columnlen && config.columnlen[columnid]) {
        dobCol.width = config.columnlen[columnid] / 8;
      }
      // }
